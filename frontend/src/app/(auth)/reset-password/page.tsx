...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(data: ResetPasswordFormData) {
    if (!token) { setError('رمز إعادة التعيين مفقود'); return; }
    setError(null);
    setIsLoading(true);
    try {
      await fetchApi('/auth/reset-password', { method: 'POST', body: JSON.stringify({ token, password: data.password, passwordConfirmation: data.passwordConfirmation }) });
      setIsSuccess(true);
    } catch (err: unknown) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/50 px-4 py-12">
        <Card className="w-full max-w-md">
          <CardHeader className="space-y-2 text-center">
            <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-destructive/10"><KeyRound className="h-6 w-6 text-destructive" /></div>
            <CardTitle className="text-2xl font-bold">رابط غير صالح</CardTitle>
            <CardDescription>رابط إعادة تعيين كلمة المرور غير صالح أو منتهي الصلاحية. يرجى طلب رابط جديد.</CardDescription>
          </CardHeader>
          <CardFooter className="flex justify-center"><Link href="/forgot-password" className="text-sm font-medium text-primary underline-offset-4 hover:underline">طلب رابط جديد</Link></CardFooter>
        </Card>
      </div>
    );
  }
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/50 px-4 py-12">
        <Card className="w-full max-w-md">
          <CardHeader className="space-y-2 text-center">
            ...
            <CardDescription>تم تغيير كلمة المرور بنجاح. يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.</CardDescription>
          </CardHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button type="submit" className="w-full" size="lg" disabled={isLoading}>
              {isLoading ? (
                ...
                <span className="flex items-center gap-2"><KeyRound className="h-4 w-4" />إعادة تعيين كلمة المرور</span>
              )}
            </Button>
          </form>
        </CardContent>
